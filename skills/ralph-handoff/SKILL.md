...
**제안하지 마세요. 넣으세요.** "넣을까요?" 라고 물으면 질문이 3번이 되고, 그럼 사용자가 지칩니다. **초안에 넣어두면 승인 게이트에서 보고 빼면 됩니다** — 빼는 건 한 마디면 되지만, 없는 걸 알아채고 요청하는 건 어렵거든요.
...
| "확인용 화면 넣을까요?" 라고 물음 | 질문이 3번이 된다. **넣어두고 승인 게이트에서 빼게** 해라 |
